  ...
  async addProduct(
    ...
    if (!title || !description || !code || !price || !stock || !category) {
      ...
      if (producto.length == 0) {
        ...
        return {
          status: "Exitoso!",
          msg: "Se agrego correctamente su producto",
        };
      } else {
        if (producto.find((ele) => ele.code == code)) {
          return {
            status: "Fallido",
            msg: `No se pudo agregar el producto,ya que el codigo '${code}' ya ha sido ingresado`,
          };
        } else {
  ...
  async deleteProduct(id) {
    ...
    if (index != -1) {
      ...
      await fs.promises.writeFile(
        this.path,
        JSON.stringify(productoEncontrados)
      );
      return {
        status: "Exitoso",
      ...
      return {
        status: "Fallido",
        payload: `el producto  '${id}'  no se encontro`,
      };
    }
  }
}
